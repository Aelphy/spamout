...
class Shingler {
    ...
    private function normalize($document) {
        ...
        $stop_words_pattern = '/\b(' . implode('|', $this->stop_words) . ')\b/';
    ...
    public function transform($document, $shingle_size) {
        $tokens = $this->normalize($document);
        $shingles = array();
        
        $vocabulary_size = count($tokens);
        if ($shingle_size > $vocabulary_size) {
            $shingle_size = $vocabulary_size;
        }

        for ($i = 0; $i < $vocabulary_size - $shingle_size; $i++) {
            $shingles[] = implode(' ', array_slice($tokens, $i, $shingle_size));
        }
